...
def return_key():
	list_directory = tools.list_dir('rsa')
	filename = './rsa/' + list_directory[0]
...
def upload_key():
	...
	if request.method == 'POST':
		...
		if file and allowed_file(file.filename):
			file.save(os.path.join("./key", file.filename))
			filename = secure_filename(file.filename)
			f1= open("./key/"+file.filename,"rb")
			f2 =open("./rsa/private.pem","rb")
			f1_data = f1.read()
			f2_data = f2.read()
			if f1_data ==f2_data :
				status = True 
